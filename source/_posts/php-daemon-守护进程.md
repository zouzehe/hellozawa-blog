---
title: php daemon 守护进程
date: 2018-05-08 23:52:25
tags: php
---
工作中经常会有业务需求需要一些守护进程脚本，实现的方式有很多，以下介绍一种pcntl
...
abstract class Daemon
{
    ...
    public function __construct()
    {
        $defaults = array(
            ...
            'maxMemory' => 0,
            'limitMemory' => 0,
            'log' => '/var/log/daemon/' . get_class($this) . '.log',
            'pid' => '/var/log/daemon/' . get_class($this) . '.pid',
    ...
    {
        global $argv;
        if (empty($argv[2]) || !in_array($argv[2], array('start', 'stop', 'restart', 'status', 'help'))) {
            $argv[2] = 'help';
        }
